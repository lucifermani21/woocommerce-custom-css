<?php 
/** WooCommerce Custom Hooks Setting **/

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
if( !class_exists( 'MS_Custom_hooks_Setting' ) ){
    class MS_Custom_hooks_Setting{
        function plugin_hooks(){
            add_action( 'after_setup_theme', array( $this, 'ms_pluign_woocommerce_support' ) );
            add_action( 'woocommerce_before_main_content', array( $this, 'MS_plugin_wrapper_start' ), 10);
            add_action( 'woocommerce_after_main_content', array( $this, 'MS_plugin_wrapper_end' ), 10);
            add_filter( 'use_block_editor_for_post_type', array( $this, 'MS_use_block_editor_for_post_type' ), 10, 2 );
            add_filter( 'woocommerce_product_add_to_cart_text', array( $this, 'MS_button_text' ) );
            add_action( 'woocommerce_before_cart_table', array( $this, 'wp_ms_woocommerce_before_cart_table_action' ) );
        ...
        function MS_plugin_wrapper_start(){
        ...
        function MS_plugin_wrapper_end(){
        ...
        function MS_use_block_editor_for_post_type( $current_status, $post_type ){
        ...
        function wp_ms_woocommerce_before_cart_table_action(){
		...
		function MS_woocommerce_product_related_products_heading($default_text){
		...
		function MS_woocommerce_return_to_shop_text( $btn_default_text ){
		...
		function MS_order_received_text( $text_html, $order ){
...
$obj = new MS_Custom_hooks_Setting;
$obj->plugin_hooks();
